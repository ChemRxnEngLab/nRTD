...
class Calibration:
    ...
    def calc_RSF(self) -> npt.NDArray:
        ...
        zero_denominator_indices = (RSF_all != 0).sum(0) == 0
        if np.any(zero_denominator_indices):
            print("Division by zero occurred in the following columns:")
            print(np.where(zero_denominator_indices)[0])
            
        print("RSF_all.sum(0):", RSF_all.sum(0))
        print("(RSF_all != 0).sum(0):", (RSF_all != 0).sum(0))
        
        RSF_all_count = (RSF_all != 0).sum(0)

        RSF = np.zeros_like(RSF_all)
        # Avoid division by zero
        non_zero_indices = RSF_all_count != 0
        RSF[:, non_zero_indices] = RSF_all[:, non_zero_indices] / RSF_all_count[non_zero_indices]
...
class MSData:
    ...
    def cal_amount(self):
        ...
        # Argon
        nt_Ar = (
            ms_data_exp[:, ms_header_exp == "Ar"]
            / self.calib.RSF[
                self.calib.components == "Ar", self.calib.ms_header_cal == "Ar"
            ]
        )
        # Helium
        nt_He = (
            ms_data_exp[:, ms_header_exp == "He"]
            / self.calib.RSF[
                self.calib.components == "He", self.calib.ms_header_cal == "He"
            ]
        )
        # Hydrogen
        nt_H2 = (
            ms_data_exp[:, ms_header_exp == "H2"]
            / self.calib.RSF[
                self.calib.components == "H2", self.calib.ms_header_cal == "H2"
            ]
        )
...
def calc_calibration(cal_files):
    ...
    # not included components can be omitted
        ...
        #eval_at[np.where(comp=='Ar')[0][0],np.where(ms_header_cal=='Ar')[0][0]] = 1 # Ar,20
        eval_at[
...
#with np.errstate(divide='ignore', invalid='ignore'):
    ...
    RSF_all_sum = RSF_all.sum(0)
    RSF_all_count = (RSF_all != 0).sum(0)
    
    print("RSF_all_sum:", RSF_all_sum)
    print("RSF_all_count:", RSF_all_count)
    print("RSF after handling division by zero:", RSF)
...
def cal_composition(path_exp, RSF, ms_header_cal):
    ...
    ms_times = np.array(_t,dtype=np.datetime64)

    # Caclulate each component
    # Argon
    nt_Ar = (
        ms_data_exp[:, np.where(ms_header_exp == "Ar")[0][0]]
        / RSF[np.where(comp == "Ar")[0][0], np.where(ms_header_cal == "Ar")[0][0]]
    )
    # Helium
    nt_He = (
        ms_data_exp[:, np.where(ms_header_exp == "He")[0][0]]
        / RSF[np.where(comp == "He")[0][0], np.where(ms_header_cal == "He")[0][0]]
    )
    # Hydrogen
    nt_H2 = (
...
def main():
    ...
    nt_all, x_i, ms_time_elap = cal_composition(path_exp, RSF, ms_header_cal)
