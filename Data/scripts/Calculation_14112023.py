...
input_filepaths = filedialog.askopenfilenames(initialdir=r'/Users/tuanaoyuncu/Desktop/RI_Data_ExpSet1',title='Select Calibration Files')
...
def calc_calibration(cal_files):
    ...
    for path_cal in cal_files:
        ...
        ms_header_cal = np.loadtxt(path_cal,delimiter='\t',skiprows=2,usecols=(1,3,5),dtype=str,max_rows=1)
        ms_header_cal = np.char.replace(ms_header_cal, '"', '')
        ms_data_cal = np.loadtxt(path_cal,delimiter='\t',skiprows=3,usecols=(1,3,5))
        ...
        Idot_ref = ms_data_avg_cal[np.where(ms_header_cal=='Mass 4')[0][0]] # Idot_ref = Mass 4
        ion_ref = He_ion
        ...
        eval_at[np.where(comp=='Ar')[0][0],np.where(ms_header_cal=='Mass 40')[0][0]] = 1 # Ar,40
        eval_at[np.where(comp=='He')[0][0],np.where(ms_header_cal=='Mass 4')[0][0]] = 1 # He,4
        eval_at[np.where(comp=='H2')[0][0],np.where(ms_header_cal=='Mass 2')[0][0]] = 1 # H2,2
        #eval_at[np.where(comp=='CO')[0][0],np.where(ms_header_cal=='Mass 28')[0][0]] = 1 # CO,28
        #eval_at[np.where(comp=='CH4')[0][0],np.where(ms_header_cal=='Mass 15')[0][0]] = 1 # CH4,15
        ...
        #eval_at[np.where(comp=='H2O')[0][0],np.where(ms_header_cal=='Mass 18')[0][0]] = 1 # H2O,18
        ...
        #eval_at[np.where(comp=='Ne')[0][0],np.where(ms_header_cal=='Mass 20')[0][0]] = 1 # Ne,20
    ...
    if non_calib[np.where(comp=='Ar')[0][0]]: # Ar not calibrated
        RSF[np.where(comp=='Ar')[0][0],np.where(ms_header_cal=='Mass 20')[0][0]] = 1*Ar_ion/ion_ref # Ar,20
        RSF[np.where(comp=='Ar')[0][0],np.where(ms_header_cal=='Mass 40')[0][0]] = 0.1*Ar_ion/ion_ref # Ar,40
    if non_calib[np.where(comp=='He')[0][0]]: # He not calibrated
        RSF[np.where(comp=='He')[0][0],np.where(ms_header_cal=='Mass 4')[0][0]] = 1*He_ion/ion_ref
    if non_calib[np.where(comp=='H2')[0][0]]: # H2 not calibrated
        RSF[np.where(comp=='H2')[0][0],np.where(ms_header_cal=='Mass 2')[0][0]] = 1*H2_ion/ion_ref
    #if non_calib[np.where(comp=='CO')[0][0]]: # CO not calibrated
     #   RSF[np.where(comp=='CO')[0][0],np.where(ms_header_cal=='Mass 28')[0][0]] = 1*CO_ion/ion_ref
    #if non_calib[np.where(comp=='CH4')[0][0]]: # CH4 not calibrated
     #   RSF[np.where(comp=='CH4')[0][0],np.where(ms_header_cal=='Mass 15')[0][0]] = 0.85*CH4_ion/ion_ref # CH4 at Mass 15
     ...
     #   RSF[np.where(comp=='H2O')[0][0],np.where(ms_header_cal=='Mass 16')[0][0]] = 0.02*H2O_ion/ion_ref # H2O at Mass 16
      #  RSF[np.where(comp=='H2O')[0][0],np.where(ms_header_cal=='Mass 18')[0][0]] = 1*H2O_ion/ion_ref # H2O at Mass 18
    ...
    #     RSF[np.where(comp=='C3')[0][0],np.where(ms_header_cal=='Mass 43')[0][0]] = 0.22*C3H8_ion/ion_ref # C3 at Mass 43
     ...
     #   RSF[np.where(comp=='Ne')[0][0],np.where(ms_header_cal=='Mass 20')[0][0]] = 1*Ne_ion/ion_ref
...
print(path_exp)
def cal_compostion(exp_data,RSF,ms_header_cal):
    print('Calculating the compositions of the given experiment...')
    
    ms_header_exp = np.loadtxt(path_exp,delimiter='\t',skiprows=2,usecols=(1,2,3,4,5,6),dtype=str,max_rows=1)
    ms_header_exp = np.char.replace(ms_header_exp, '"', '')
    ms_data_exp = np.loadtxt(path_exp,delimiter='\t',skiprows=3,usecols=(1,2,3,4,5,6))
    # read the measurement time
    print('Reading the measurement time...')
    ms_time_exp = np.loadtxt(path_exp,delimiter='\t',skiprows=3,usecols=(0),dtype=str)
    ...
    # ax.plot(ms_time_elap,ms_data_exp[:,np.where(ms_header_exp=='Mass 40')[0][0]+1],label='Mass 58, vorher')
    ...
    # ms_data_exp[:,np.where(ms_header_exp=='Mass 40')[0][0]+1] = ms_data_exp[:,np.where(ms_header_exp=='Mass 20')[0][0]+1] - 0.89/100*ms_data_exp[:,np.where(ms_header_exp=='Mass 20')[0][0]]
    # # ax.plot(ms_time_elap,ms_data_exp[:,np.where(ms_header_exp=='Mass 4')[0][0]+1],label='Mass 43, nachher')
    # ax.plot(ms_time_elap,ms_data_exp[:,np.where(ms_header_exp=='Mass 40')[0][0]+1],label='Mass 58, nachher')
    # ax.legend()
    # plt.show()
    ...
    # Argon
    nt_Ar = ms_data_exp[:,np.where(ms_header_exp=='Mass 40')[0][0]]/RSF[np.where(comp=='Ar')[0][0],np.where(ms_header_cal=='Mass 40')[0][0]]
    # Helium
    nt_He = ms_data_exp[:,np.where(ms_header_exp=='Mass 4')[0][0]]/RSF[np.where(comp=='He')[0][0],np.where(ms_header_cal=='Mass 4')[0][0]]
    # Hydrogen
    nt_H2 = ms_data_exp[:,np.where(ms_header_exp=='Mass 2')[0][0]]/RSF[np.where(comp=='H2')[0][0],np.where(ms_header_cal=='Mass 2')[0][0]]
    # Methane
    #nt_CH4 = ms_data_exp[:,np.where(ms_header_exp=='Mass 15')[0][0]]/RSF[np.where(comp=='CH4')[0][0],np.where(ms_header_cal=='Mass 15')[0][0]]
    # Water
    #nt_H2O = ms_data_exp[:,np.where(ms_header_exp=='Mass 18')[0][0]]/RSF[np.where(comp=='H2O')[0][0],np.where(ms_header_cal=='Mass 18')[0][0]]
    # Ethane
    ...
    # CO
    #Idot28 = ms_data_exp[:,np.where(ms_header_exp=='Mass 28')[0][0]]
    ...
    #RSF_C2_28 = RSF[np.where(comp=='C2')[0][0],np.where(ms_header_cal=='Mass 28')[0][0]]
    ...
    #RSF_C3_28 = RSF[np.where(comp=='C3')[0][0],np.where(ms_header_cal=='Mass 28')[0][0]]
    # RSF_C3_43 = RSF[np.where(comp=='C3')[0][0],np.where(ms_header_cal=='Mass 43')[0][0]]
    #RSF_CO_28 = RSF[np.where(comp=='CO')[0][0],np.where(ms_header_cal=='Mass 28')[0][0]]
    # if gc_prompt == 'yes':
    ...
    #nt_CO = Idot28/RSF_CO_28 # without C2 and C3
    # Ne
    #Idot20 = ms_data_exp[:,np.where(ms_header_exp=='Mass 20')[0][0]]
    #Idot40 = ms_data_exp[:,np.where(ms_header_exp=='Mass 40')[0][0]]
    #RSF_Ar_20 = RSF[np.where(comp=='Ar')[0][0],np.where(ms_header_cal=='Mass 20')[0][0]]
    #RSF_Ar_40 = RSF[np.where(comp=='Ar')[0][0],np.where(ms_header_cal=='Mass 40')[0][0]]
    #RSF_Ne_20 = RSF[np.where(comp=='Ne')[0][0],np.where(ms_header_cal=='Mass 20')[0][0]]
   ...
   # try:
    #    np.where(ms_header_exp == 'Mass 30')[0][0]
     #   RSF_C2_30 = np.array([])
      #  mass26 = False
  #  except:
   #     np.where(ms_header_exp == 'Mass 26')[0][0]
    #    RSF_C2_26 = np.array([])
     #   mass26 = True
   # try:
   #     np.where(ms_header_exp == 'Mass 43')[0][0]
    #    RSF_C3_43 = np.array([])
"""except:
    ...
    for t in range(1,len(time_str)):
        try:
            ...
            try:
                ...
                Idot_26 = ms_data_avg_gc[np.where(ms_header_exp == 'Mass 26')][0]
            try:
                Idot_43 = ms_data_avg_gc[np.where(ms_header_exp == 'Mass 43')][0]
            except:
                Idot_44 = ms_data_avg_gc[np.where(ms_header_exp == 'Mass 44')][0]
            Idot_58 = ms_data_avg_gc[np.where(ms_header_exp == 'Mass 58')][0]
            # Choose reference
            # print('\nHe as reference')
            x_ref = x_i[np.where(comp=='He')[0][0]-1,avg_idx] # x_ref = x_He as reference
            # print('Reference mole fraction:',x_ref)
            Idot_ref = ms_data_avg_gc[np.where(ms_header_exp=='Mass 4')[0][0]] # Idot_ref = Mass 4
...
#             print('064 erkannt')
#             nt_C2 = ms_data_exp[:,np.where(ms_header_exp=='Mass 26')[0][0]]/2 # RSF C2 Mass 26 = 2.0
#         else:
#             nt_C2 = ms_data_exp[:,np.where(ms_header_exp=='Mass 26')[0][0]]/RSF_C2_26[gc_ind]  
# else:
#     # nt_C2 = ms_data_exp[:,np.where(ms_header_exp=='Mass 30')[0][0]]/RSF[np.where(comp=='C2')[0][0],np.where(ms_header_cal=='Mass 30')[0][0]]
#     nt_C2 = ms_data_exp[:,np.where(ms_header_exp=='Mass 26')[0][0]]/RSF[np.where(comp=='C2')[0][0],np.where(ms_header_cal=='Mass 26')[0][0]]
# # Propane
# if gc_prompt == 'yes':
#     try:
#         print('RSF C3 Mass 43:',RSF_C3_43[gc_ind])
#         if path_exp == r'C:/Users/simon/OneDrive - bwedu/Studium/02_Master/01_Masterarbeit/02_Versuche/03_Rohdaten/MS/SHA-E-064_260C_4_H2CO_20230204092754.txt': # by hand RSF for SHA-E-064
#             nt_C3 = ms_data_exp[:,np.where(ms_header_exp=='Mass 43')[0][0]]/20 # RSF C3plus Mass 43 = 30
#             print('064 erkannt')
#         else:
#             nt_C3 = ms_data_exp[:,np.where(ms_header_exp=='Mass 43')[0][0]]/RSF_C3_43[gc_ind] 
#     except:   
...
# if gc_prompt == 'yes':
#     if path_exp == r'C:/Users/simon/OneDrive - bwedu/Studium/02_Master/01_Masterarbeit/02_Versuche/03_Rohdaten/MS/SHA-E-064_260C_4_H2CO_20230204092754.txt': # by hand RSF for SHA-E-064
#         nt_C4 = ms_data_exp[:,np.where(ms_header_exp=='Mass 58')[0][0]]/2 # RSF C4plus Mass 58 = 2
#         print('064 erkannt')
#     else:
#         nt_C4 = ms_data_exp[:,np.where(ms_header_exp=='Mass 58')[0][0]]/RSF_C4_58[gc_ind]
...
RSF,ms_header=calc_calibration(input_filepaths)
nt,x_i, ms_time_elap=cal_compostion(path_exp,RSF,ms_header)
