...
switching_times_reac_outlet = np.insert(automated_switching_times, 0, 8.8 - 0.5 - 1.0)
switching_times_reac_outlet = np.cumsum(switching_times_reac_outlet)
switching_times_total_system = np.insert(automated_switching_times, 0, 12.9 - 0.5 - 1.0)
switching_times_total_system = np.cumsum(switching_times_total_system)

t_kernels = [
    (0.0, 35.0),  # piping to reactor inlet
    (0.0, 50.0),  # reactor
    (0.0, 10.0),  # piping after reactor outlet
    (0.0, 20.0),  # capillary
]
...
#     model.net.conv_layers[3].conv.weight, mean=0.2, std=0.1, a=0.1, b=1.0
# )
torch.nn.init.zeros_(model.net.conv_layers[3].conv.weight)
...
# torch.nn.init.trunc_normal_(
#     model.net.conv_layers[0].conv.weight, mean=0.2, std=0.1, a=0.1, b=1.0
# )
torch.nn.init.zeros_(model.net.conv_layers[0].conv.weight)
...
# torch.nn.init.trunc_normal_(
#     model.net.conv_layers[1].conv.weight, mean=0.2, std=0.1, a=0.1, b=1.0
# )
torch.nn.init.zeros_(model.net.conv_layers[1].conv.weight)
model.configure_optimizers()
trainer = pl.Trainer(
    accelerator="cpu",
    max_epochs=max_epochs,
...
# torch.nn.init.trunc_normal_(
#     model.net.conv_layers[2].conv.weight, mean=0.2, std=0.1, a=0.1, b=1.0
# )
torch.nn.init.zeros_(model.net.conv_layers[2].conv.weight)
model.configure_optimizers()
trainer = pl.Trainer(
    accelerator="cpu",
    max_epochs=max_epochs,
