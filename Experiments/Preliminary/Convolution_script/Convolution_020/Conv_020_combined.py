...
laminar_model_dir = os.path.expanduser('~/Documents/GitHub/nRTD/Experiments/Preliminary/Convolution_script/Convolution_018_Laminar_Model')
adler_dir = os.path.expanduser('~/Documents/GitHub/nRTD/Experiments/Preliminary/Litrature/Adler_havarka_Model/Combined_tau_a_val_1_tau_p_val_2_tau_m_val_0.4000000000000001_beta_val_0.1')

print("Laminar Model Directory Path:", laminar_model_dir)
print("Adler Model Directory Path:", adler_dir)

assert os.path.exists(os.path.join(adler_dir, 'time.npy'))
assert os.path.exists(os.path.join(adler_dir, 'concentration.npy'))
assert os.path.exists(os.path.join(laminar_model_dir, 't_E_predicted.npy'))
assert os.path.exists(os.path.join(laminar_model_dir, 'c_conv_in_100.npy'))
...
t_in_conv= torch.linspace(0, 30, 50).unsqueeze(0)
c_in_conv = torch.tensor(np.load(os.path.join(laminar_model_dir, 'c_conv_in_100.npy')), dtype=torch.float32)
...
fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(10, 8))
fig.subplots_adjust(hspace=0)  
ax1.plot(t_in_conv.squeeze(0).numpy(), c_in_conv.squeeze(0).squeeze(0).numpy(), label="SF", color="blue")
for i in range(c_out.size(1)):
    ax1.plot(
        t_conv[0, i, :].numpy(),
        c_out[0, i, :].numpy(),
...
unified_dir = os.path.join(save_dir, f'tau_a_val_{1}_tau_p_val_2')
os.makedirs(unified_dir, exist_ok=True)
...
predicted_E = np.load('/Users/tuanaoyuncu/Documents/GitHub/nRTD/Experiments/Preliminary/Convolution_script/Convolution_020/tau_a_val_1_tau_p_val_2/E_predicted_tau_a_val_1.npy')
predicted_time = np.load('/Users/tuanaoyuncu/Documents/GitHub/nRTD/Experiments/Preliminary/Convolution_script/Convolution_020/tau_a_val_1_tau_p_val_2/t_E_predicted_tau_a_val_1.npy')
...
plt.savefig(os.path.join(unified_dir, 'E_saved_10102024_tau_a_val_1.png'), dpi=300)
plt.show()
