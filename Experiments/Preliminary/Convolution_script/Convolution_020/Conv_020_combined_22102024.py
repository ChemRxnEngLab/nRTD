...
t_in_conv= torch.linspace(0, 30, 200).unsqueeze(0).unsqueeze(0)
c_in_conv = torch.tensor(np.load(os.path.join(laminar_model_dir, 'c_conv_in_combined.npy')), dtype=torch.float32)
...
fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(10, 8))
fig.subplots_adjust(hspace=0)  
ax1.plot(t_in_conv.squeeze().numpy(), c_in_conv.squeeze().squeeze().numpy(), label="SF", color="blue")
for i in range(c_out.size(1)):
    ax1.plot(
        t_conv[0, i, :].numpy(),
        c_out[0, i, :].numpy(),
