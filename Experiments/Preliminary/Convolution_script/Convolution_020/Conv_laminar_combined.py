...
tau_5_dir = '/Users/tuanaoyuncu/Documents/GitHub/nRTD/Experiments/Preliminary/Litrature/Laminar_Flow_Model/tau_combined_5.0'
t_conv_tau = torch.tensor(np.load(os.path.join(tau_5_dir, 'time_combined.npy')), dtype=torch.float32).unsqueeze(0).unsqueeze(0)
c_out_tau = torch.tensor(np.load(os.path.join(tau_5_dir, 'concentration_combined.npy')), dtype=torch.float32).unsqueeze(0).unsqueeze(0)
...
ax2.legend()
plt.xlabel("Time")
plt.savefig('Figure_connected_plot_21102024_combined.png', dpi=300)
plt.show()
...
c_conv_in_combined=c_conv.detach().numpy()
...
np.save(os.path.join(save_dir, 'E_predicted_combined.npy'), predicted_E)
np.save(os.path.join(save_dir, 't_E_predicted_combined.npy'), predicted_time)
np.save(os.path.join(save_dir, 'E_expected_combined.npy'), expected_E)
np.save(os.path.join(save_dir, 't_E_expected_combined.npy'), expected_time)
np.save(os.path.join(save_dir, 'c_conv_in_combined.npy'),c_conv_in_combined )
...
plt.ylim((0, 1.1))  
plt.legend()
plt.savefig('E_saved_21102024_combined.png', dpi=300)
plt.show()
print("expected_time",expected_time)
print(f"c_conv_in size: {t_E.numpy().shape}")
print(f"c_conv_in size: {t_E_np.shape}")
print(f"c_conv_in_combined shape: {c_conv_in_combined.shape}")
