...
class MyPrintingCallback(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        self.losses = np.append(
            self.losses, trainer.callback_metrics["train/loss"].cpu().detach().numpy()
        )
        print(self.losses.shape)
        update_plots(self.losses)
...
adler_dir = (
    ...
    / "Experiments"
    / "Preliminary"
    / r"Litrature\Adler_havarka_Model\tau_a_val_3_tau_p_val_2_tau_m_val_0.4000000000000001_beta_val_0.1_26_11"
)

print(adler_dir)
print(adler_dir.exists())
...
(
    ...
    n_out_1,
    n_e_1,
) = sp.symbols("n_in_1 n_out_1 n_e_1 ", positive=True, real=True)
t_i, t_o, t_e_1 = sp.symbols("t_i, t_o, t_e_1 ", positive=True, real=True)
equations = [
    t_i - t_o + t_e_1,
...
for equation in equations:
    print(equation)
solution_set = sp.solve(equations, (n_in_1, n_out_1, n_e_1, t_o), dict=True)
print(solution_set)
...
t_conv_tau = (
    torch.tensor(np.load(os.path.join(adler_dir, "time.npy")), dtype=torch.float32)
    .unsqueeze(0)
    .unsqueeze(0)
)
c_out_tau = (
    torch.tensor(
        np.load(os.path.join(adler_dir, "concentration.npy")), dtype=torch.float32
    )
    .unsqueeze(0)
    .unsqueeze(0)
)
...
fig, axs = plt.subplot_mosaic([["B", "A"], ["C", "A"]], figsize=(10, 8))
axs = [axs["A"], axs["B"], axs["C"]]
axs[0].set(
    xlabel="t / s",
    ylabel="c / 1",
)
line_c0 = axs[0].plot(t_input, c_in[j, 0, :].numpy(), label="SF", color="blue")
for i in range(c_out.size(1)):
    axs[0].plot(
        t_conv[j, i, :].numpy(), c_out[j, i, :].numpy(), label="Tau 5.0", color="green"
    )
line_c_out = axs[0].plot(
    t_conv[j, 0, :].numpy(),
    c_conv[j, 0, :].detach().numpy(),
    label="Predicted",
    color="red",
)
line_E = axs[1].plot(t_E, E, label="E", color="orange")
axs[1].set(
    xlabel="t / s",
    ylabel="E / 1",
)
line_loss = axs[2].plot([], [])
axs[2].set(
    xlabel="epoch",
    ylabel="loss",
    yscale="log",
...
def update_plots(losses):
    E = model.net.E[0]
    E = E / E.max()
    c_conv = model(c_in.to(model.device))
    line_E[0].set_ydata(E)
    line_c_out[0].set_ydata(c_conv[j, 0, :].cpu().detach().numpy())
    line_loss[0].set_xdata(np.arange(len(losses)))
    line_loss[0].set_ydata(losses)
    axs[2].set_xlim((0, 1.1 * len(losses)))
    fig.canvas.draw()
...
fig, (ax1, ax2) = plt.subplots(
    2, 1, sharex=True, figsize=(Elsevier_Sizes.double_column["in"], 12 * cm2inch)
)
ax1.plot(
    t_input.numpy(), c_in[0, 0, :].numpy(), label=r"$x_0(t)$", color=ICIWcolors.CERULEAN
)
for i in range(c_out.size(1)):
    ax1.plot(
        t_conv[0, i, :].numpy(),
...
current_date = datetime.datetime.now().strftime("%Y%m%d")
plt.savefig(os.path.join(save_dir, f"Profiles_tau_3_{current_date}.png"), dpi=300)
plt.show()
