...
t_conv_tau = torch.tensor(np.load(os.path.join(tau_5_dir, 'time_J5_tau3_alpha0.2_200.npy')), dtype=torch.float32).unsqueeze(0).unsqueeze(0)
c_out_tau = torch.tensor(np.load(os.path.join(tau_5_dir, 'concentration_J5_tau3_alpha0.2_200.npy')), dtype=torch.float32).unsqueeze(0).unsqueeze(0)
...
E_t = (0.507094 * np.exp(-1.74903 * t_plot) * t_plot**4
...
ax2.set_ylim((-0.1, 1.1)) 
current_date = datetime.datetime.now().strftime("%Y%m%d")
plt.savefig(f"Profiles_5_3{current_date}.png", dpi=300)
plt.show()
