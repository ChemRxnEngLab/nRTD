...
t_conv_tau = torch.tensor(np.load(os.path.join(tau_5_dir, 'time_J5_tau1_alpha0.2_200.npy')), dtype=torch.float32).unsqueeze(0).unsqueeze(0)
c_out_tau = torch.tensor(np.load(os.path.join(tau_5_dir, 'concentration_J5_tau1_alpha0.2_200.npy')), dtype=torch.float32).unsqueeze(0).unsqueeze(0)
...
E_t = (123.224 * np.exp(-5.24709 * t_plot) * t_plot**4
