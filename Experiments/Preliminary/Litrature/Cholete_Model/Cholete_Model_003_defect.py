...
def Cholete(t: npt.NDArray[np.float64], alpha: float, beta: float, tau: float, g: float) -> npt.NDArray[np.float64]: 
    ...
    E[E < 0] = 0
    #E /= np.trapz(E, t)  # Normalize by integral of E
    return E
...
# Load the data after saving and print shapes
for beta in beta_values:
    beta_dir = os.path.join(base_dir, f'beta{beta}')
    t_conv = np.load(os.path.join(beta_dir, 'time.npy'))  # Correct loading path
    c_out = np.load(os.path.join(beta_dir, 'concentration.npy'))  # Correct loading path
    print(f"Results for beta = {beta}:")
    print(f"Time array shape: {t_conv.shape}")
    print(f"Concentration array shape: {c_out.shape}")
    print(f"First 10 Time values: {t_conv[:10]}...")
    print(f"First 10 Concentration values: {c_out[:10]}...\n")
