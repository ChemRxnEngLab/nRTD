...
for disc in discretization_Laminar:
    ...
    ax2.plot(t_conv_l, c_out_l_noisy, label=f'Tau {tau_l} (Noisy, Q-Squared)', color='red', linestyle='--')
    ax1.set_xlim(0, 50)
    ax1.set_xlabel('Time')
    ax1.set_ylabel('E')
    ax1.legend()
    ...
    plt.savefig(os.path.join(tau_l_dir, f'Laminar_Flow_Model_{current_date}_Disc_{disc}_q_squared_noisy.png'), dpi=300)
    plt.show()
