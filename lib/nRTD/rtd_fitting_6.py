...
class RTDDataModule(pl.LightningDataModule):
    # adjusted from the previous version to get a better mean of initial and end conditions
    def __init__(
        self,
        batch_size: int,
        data_file: Path,
    ...
    def setup(self, stage: str):
        ...
        # calculate the mean of the steady state values
        even_steady_state_values = np.array(
            [
                [
                    n_dot_interp[0::4, 0:4].mean(),  # BU Ar beg
                    n_dot_interp[0::4, -5:-1].mean(),  # BU Ar end
                ],
                [
                    n_dot_interp[1::4, 0:4].mean(),  # BU He beg
                    n_dot_interp[1::4, -5:-1].mean(),  # BU He end
                ],
            ]
        )

        odd_steady_state_values = np.array(
            [
                [
                    n_dot_interp[2::4, 0:4].mean(),  # BT Ar
                    n_dot_interp[2::4, -5:-1].mean(),  # BT Ar
                ],
                [
                    n_dot_interp[3::4, 0:4].mean(),  # BT He
                    n_dot_interp[3::4, -5:1].mean(),  # BT He
                ],
            ]
        )
