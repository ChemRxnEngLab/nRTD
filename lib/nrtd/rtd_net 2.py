from typing import Optional
import torch.nn as nn
import torch
import numpy.typing as npt

class RTDNet(nn.Module):
    """RTDNet with one convolution and one deconvolution layer.

    Parameters
    ----------
    nn : _type_
        _description_
    """

    def __init__(
        self,
        kernel_size: int,
        padding_mode: str = "replicate",
        n_compartements: int = 1,
        t_conv: Optional[list[tuple[float, float]]] = None,
    ):
        super().__init__()
        self.n_compartements = n_compartements
        self.kernel_size = kernel_size
        self.padding_mode = padding_mode

        # Convolution Layer
        self.conv_layer = nn.Conv1d(
            in_channels=1,
            out_channels=1,
            kernel_size=kernel_size,
            bias=False,  # no offset
            padding=self.kernel_size,  # pad to make convolution causal/full
            padding_mode=self.padding_mode,
        )

        # Deconvolution (Transposed Convolution) Layer
        self.deconv_layer = nn.ConvTranspose1d(
            in_channels=1,
            out_channels=1,
            kernel_size=kernel_size,
            bias=False,
            padding=self.kernel_size,
        )

        if t_conv is None:
            self.t_conv = [(0.0, 10.0) for i in range(n_compartements)]
        else:
            self.t_conv = t_conv

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through the network: convolution + deconvolution.

        Parameters
        ----------
        x : torch.Tensor
            input data

        Returns
        -------
        torch.Tensor
            output data (Deconvolved E function)
        """
        conv_out = self.conv_layer(x)  # Apply convolution
        deconv_out = self.deconv_layer(conv_out)  # Apply deconvolution

        # Here, deconv_out is treated as the E function, which is our deconvolved signal
        return deconv_out

    def _freeze_conv(self, ind: int) -> None:
        """Utility function for freezing the weights of the convolutional layer.

        Parameters
        ----------
        ind : int
            index of the convolutional layer to freeze.
        """
        self.conv_layer.weight.requires_grad = False

    @property
    def E(self) -> list[npt.NDArray]:
        """RTD density functions for compartments.

        Returns
        -------
        list[npt.NDArray]
            RTD density functions for compartments
        """
        return [
            self.conv_layer.weight[0, 0, :].flip(0).detach().numpy()
        ]

    def output_shape(self, c_in: torch.Tensor) -> int:
        """The output shape (discretization) of the RTD convolution function if the input shape is 'c_in'.

        Parameters
        ----------
        c_in : torch.Tensor
            temporal input signal

        Returns
        -------
        int
            discretization (length) of the output signal
        """
        n_c = c_in.shape[2]
        return n_c + 2 * (self.kernel_size + 1)  # Adjusted for conv + deconv

    @property
    def t_conv_end(self) -> float:
        """
        The end time of the RTD Convolution function, as the sum of the end times of the individual compartments.

        Returns
        -------
        float
            end time of the RTD Convolution function
        """
        t_conv_end = [t[-1] for t in self.t_conv]
        return sum(t_conv_end)

    def conv_t(self, t_input: torch.Tensor) -> torch.Tensor:
        """The Output time scale of the RTD convolution function if the input time scale is 't_input'.

        Parameters
        ----------
        t_input : torch.Tensor
            time scale of the input signal.

        Returns
        -------
        torch.Tensor
            time scale of the convoluted (output) signal.
        """
        t_i_end = float(t_input[-1])
        t_c_end = t_i_end + self.t_conv_end
        return torch.linspace(0, t_c_end, self.output_shape(t_input))


if __name__ == "__main__":
    # Create model
    model = RTDNet(kernel_size=100, padding_mode="replicate", n_compartements=1)

    # Create random input tensor
    c_in = torch.rand(1, 1, 100)

    # Perform forward pass: Convolution + Deconvolution
    c_deconv = model(c_in)

    # Print the predicted E function from deconvolution
    print("Deconvolution result (E_predicted):")
    print(c_deconv)

    # Plot the deconvolved E function
    t_input = torch.linspace(0, 40, c_deconv.size(-1))